...
func init() {
	...
	ConvertMap.Set(IntTypes, IntTypes, func(c *Convertor, src, dst reflect.Value) {
		dst.SetInt(src.Int())
	})

	// FLOAT
	FloatTypes = []interface{}{reflect.Float32, reflect.Float64}
	ConvertMap.Set(FloatTypes, FloatTypes, func(c *Convertor, src, dst reflect.Value) {
		dst.SetFloat(src.Float())
	})

	// STRING
	StringTypes = []interface{}{reflect.String}
	ConvertMap.Set(StringTypes, StringTypes, func(c *Convertor, src, dst reflect.Value) {
	...
	ConvertMap.Set(TimeTypes, TimeTypes, func(c *Convertor, src, dst reflect.Value) {
		dst.Set(src)
	})

	ConvertMap.Set(TimeIntTypes, TimeTypes, func(c *Convertor, src, dst reflect.Value) {
		dst.Set(reflect.ValueOf(time.Unix(src.Int(), 0)))
	})

	ConvertMap.Set(TimeTypes, TimeIntTypes, func(c *Convertor, src, dst reflect.Value) {
		dst.SetInt(src.Interface().(time.Time).Unix())
	})

	ConvertMap.Set(StringTypes, TimeTypes, func(c *Convertor, src, dst reflect.Value) {
		t, _ := time.Parse(c.timeFormat, src.String())
		dst.Set(reflect.ValueOf(t))
	})

	ConvertMap.Set(TimeTypes, StringTypes, func(c *Convertor, src, dst reflect.Value) {
